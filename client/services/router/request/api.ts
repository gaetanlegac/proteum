...
export default class ApiClient implements ApiClientService {
  ...
  public post = <TData extends unknown = unknown>(
    path: string,
    data?: TPostData,
    opts?: TApiFetchOptions,
  ...
  public set(newData: TObjetDonnees) {
    ...
    if (this.router.context.page)
      this.router.context.page.setAllData((curData) => ({
        ...curData,
        ...newData,
      }));
  ...
  public async fetchSync(
    fetchers: TFetcherList,
    alreadyLoadedData: {},
  ): Promise<TObjetDonnees> {
    ...
    if (data) {
      ...
      if (method === "GET") {
        const params = new URLSearchParams(
          data as unknown as TPostDataWithoutFile,
        ).toString();
        url = `${url}?${params}`;
      } else if (options.encoding === "multipart") {
