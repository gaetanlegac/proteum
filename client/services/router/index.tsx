...
type TRouteLoader<Route extends TRoute | TErrorRoute = TRoute | TErrorRoute> =
  () => Promise<TRouteModule<Route>>;

export type TFetchedRoute = Pick<
  TRoute,
  "path" | "options" | "controller" | "method"
>;

export type TRoutesLoaders = {
  [chunkId: string]: () => Promise<
    /* Preloaded via require() */ | TFetchedRoute
    | /* Loader via import() */ TRouteLoader /* | undefined*/
  >;
};
...
export type THookCallback<TRouter extends ClientRouter> = (
  ...
  implements BaseRouter
{
  ...
  public go(
    ...
    if (typeof url === "number") {
      this.createResponse(this.errors[url], this.context.request, data).then(
        (page) => {
  ...
  public async registerRoutes() {
    ...
    debug &&
      console.log(
        LogPrefix,
        `Indexing routes and finding the current route from ssr data:`,
        this.context,
      );
    ...
    for (let routeIndex = 0; routeIndex < this.ssrRoutes.length; routeIndex++) {
      ...
      if (loaders[ssrRoute.chunk] === undefined) {
        console.error(
          "Chunk id not found for ssr route:",
          ssrRoute,
          "Searched in:",
          loaders,
        );
  ...
  public page(...args: TRegisterPageArgs<any, TRoute["options"]>): TRoute {
    ...
    const route: TRoute = {
      ...
      controller: (context: TClientOrServerContextForPage) =>
        new ClientPage(route, renderer, context, layout),
    };
  ...
  private async load(
    ...
    if (typeof route.load === "function") {
      ...
      try {
        ...
        throw new Error(
          "A new version of the website is available. Please refresh the page.",
        );
      }
    } else {
  ...
  private async createErrorResponse(
    ...
    if (route === undefined) {
      console.error(
        `Error page for http error code ${code} not found.`,
        this.errors,
        this.routes,
      );
      e.http = 404;
  ...
  public on(hookName: THookName, callback: THookCallback<this>) {
    ...
    return () => {
      debug &&
        console.info(
          LogPrefix,
          `De-register hook ${hookName} (index ${cbIndex})`,
        );
