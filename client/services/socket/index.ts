...
class SocketScope {
  ...
  public constructor(
    ...
    for (const event in events) this.events[event] = [events[event]];
  }
  ...
  private connect() {
    ...
    this.ws.onmessage = (event: MessageEvent<any>) => {
      ...
      for (const callback of this.events[name]) callback(data);
    };
    ...
    this.ws.onclose = (event) => {
      ...
      if (event.wasClean) {
        console.log(
          `[socket] Disconnected from ${url}. Reason: `,
          event.reason,
        );
        return;
      }
      ...
      if (!netwokrStatusToast)
        netwokrStatusToast = this.context.toast.error(
          "You're offline",
      ...
      console.log(
        `[socket] Disconnected from ${url}. Retry in 5 seconds ...`,
        event,
      );
  ...
  public on(event: string, callback: TEventCallback) {
    if (this.events[event] === undefined) this.events[event] = [callback];
