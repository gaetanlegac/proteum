...
export const Switch = (
  val: string | number,
  options: { [cle: string]: ComponentChild },
) => {
  return val in options ? options[val] : null;
};

export const useState = <TData extends TObjetDonnees>(
  initial: TData,
): [TData, (data: Partial<TData>) => void] => {
  const [state, setState] = React.useState<TData>(initial);
  const setPartialState = (data: Partial<TData>) =>
