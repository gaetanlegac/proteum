...
if (!repoRoots.length)
  throw new Error(
    "Usage: ts-node scripts/restore-client-app-import-files.ts <repo-root> [repo-root...]",
  );
...
for (const repoRoot of repoRoots) {
  ...
  const headAppImportFiles = new Set(
    grepOutput
      ...
      .filter(
        (line) =>
          line.startsWith("client/components/") ||
          line.startsWith("client/hooks/"),
      ),
  );

  const filesToRestore = currentDiffFiles.filter((filepath) =>
    headAppImportFiles.has(filepath),
  );

  for (const relativeFile of filesToRestore) {
    const nextContent = execFileSync(
      "git",
      ["-C", repoRoot, "show", `HEAD:${relativeFile}`],
      {
        encoding: "utf8",
      },
    );
