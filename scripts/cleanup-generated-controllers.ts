...
const findControllerFiles = (dir: string): string[] => {
  ...
  for (const dirent of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (dirent.isFile() && dirent.name.endsWith(".controller.ts"))
      files.push(filepath);
  }
...
if (!repoRoots.length)
  throw new Error(
    "Usage: ts-node scripts/cleanup-generated-controllers.ts <repo-root> [repo-root...]",
  );
...
for (const repoRoot of repoRoots) {
  ...
  for (const controllerFile of controllerFiles) {
    ...
    if (
      ...
      const parentName = path
        .basename(path.dirname(controllerFile))
  ...
  console.log(
    `[cleanup-generated-controllers] ${repoRoot}: updated ${updated} controller files`,
  );
}
