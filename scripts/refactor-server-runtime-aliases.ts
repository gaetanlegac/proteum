...
type TRuntimeBinding = {
  local: string;
  imported: string;
  source: TRuntimeImportSource;
};
...
const fallbackBindings = new Map<string, TRuntimeBinding>([
  ...
  [
    "Environment",
    { local: "Environment", imported: "Environment", source: "@app" },
  ],
...
const getOriginalFileContent = (repoRoot: string, filepath: string) => {
  ...
  try {
    return execFileSync(
      "git",
      ["-C", repoRoot, "show", `HEAD:${relativeFilepath}`],
      { encoding: "utf8" },
    );
  } catch {
    return null;
...
const extractRuntimeBindings = (code: string, filename: string) => {
  ...
  traverse(ast, {
    ImportDeclaration(importPath) {
      ...
      if (source !== "@app" && source !== "@request" && source !== "@models")
        return;
...
const getClassContext = (path: NodePath) => {
  const classPath = path.findParent((parentPath) =>
    parentPath.isClass(),
  ) as NodePath<types.Class> | null;
  ...
  for (const member of classNode.body.body) {
    if (
      (types.isClassProperty(member) ||
        types.isClassAccessorProperty?.(member)) &&
    ...
    if (types.isClassMethod(member) && types.isIdentifier(member.key)) {
      if (member.kind === "get" && member.key.name === "request")
        hasRequest = true;
        ...
        for (const param of member.params) {
          if (types.isTSParameterProperty(param)) {
            ...
            const identifier = types.isIdentifier(parameter)
              ? parameter
              : types.isAssignmentPattern(parameter) &&
...
const buildReplacementExpression = (
  binding: TRuntimeBinding,
  classContext: TClassContext | null,
) => {
...
if (!repoRoots.length)
  throw new Error(
    "Usage: ts-node scripts/refactor-server-runtime-aliases.ts <repo-root> [repo-root...]",
  );

for (const repoRoot of repoRoots) {
  const serviceRoot = path.join(repoRoot, "server", "services");
  const files = findFiles(serviceRoot).filter(
  ...
  for (const filepath of files) {
    ...
    traverse(ast, {
      MemberExpression(memberPath) {
        ...
        if (
          !types.isIdentifier(memberPath.node.object.property, { name: "app" })
        )
          return;
        if (!types.isIdentifier(memberPath.node.property)) return;
      ...
      Identifier(identifierPath) {
        const binding =
          runtimeBindings.get(identifierPath.node.name) ||
          fallbackBindings.get(identifierPath.node.name);
        if (!binding) return;
        if (!identifierPath.isReferencedIdentifier()) return;
        if (isInTypePosition(identifierPath)) return;

        const currentBinding = identifierPath.scope.getBinding(
  ...
  console.info(
    `[runtime-aliases] ${repoRoot}: changed ${changedFiles} files, ${replacementCount} replacements`,
  );
