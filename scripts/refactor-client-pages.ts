...
const pushUniqueObjectProperty = (
  ...
  const existingIndex = properties.findIndex(
    (existingProperty) =>
      existingProperty.type === "ObjectProperty" &&
      getObjectPropertyKeyName(existingProperty) === nextKey,
  );
...
const pushUniqueObjectPatternProperty = (
  ...
  const existingIndex = properties.findIndex(
    (existingProperty) =>
      (nextLocalName &&
        getObjectPatternPropertyLocalName(existingProperty) ===
          nextLocalName) ||
      (nextKeyName &&
        getObjectPropertyKeyName(existingProperty) === nextKeyName),
  );
...
const unwrapFetchExpression = (init: types.Expression | null | undefined) => {
  ...
  while (current) {
    ...
    if (current.type === "TSAsExpression") {
      ...
      wrappers.push((expression) =>
        types.tsAsExpression(expression, typeAnnotation),
      );
      current = current.expression;
      continue;
    }
    ...
    if (current.type === "TSTypeAssertion") {
      const typeAnnotation = current.typeAnnotation;
      wrappers.push((expression) =>
        types.tsTypeAssertion(typeAnnotation, expression),
      );
      current = current.expression;
      continue;
    }
  ...
  return {
    ...
    wrapExpression(expression: types.Expression) {
      return wrappers.reduceRight(
        (acc, applyWrapper) => applyWrapper(acc),
        expression,
      );
    },
  };
...
const prefixOptionsObject = (options: types.ObjectExpression) => {
  ...
  for (const property of options.properties) {
    ...
    if (property.key.type === "Identifier") {
      properties.push(
        types.objectProperty(
          types.identifier(`_${property.key.name}`),
          property.value,
          false,
          false,
        ),
    ...
    if (property.key.type === "StringLiteral") {
      properties.push(
        types.objectProperty(
          types.stringLiteral(`_${property.key.value}`),
          property.value,
        ),
      );
      continue;
...
if (!repoRoots.length)
  throw new Error(
    "Usage: ts-node scripts/refactor-client-pages.ts <repo-root> [repo-root...]",
  );
...
for (const repoRoot of repoRoots) {
    ...
    traverse(ast, {
      ...
      CallExpression(path) {
        ...
        if (
          ...
          const renderArg =
            path.get("arguments")[path.node.arguments.length - 1];
          if (
            renderArg &&
    ...
    if (renderFunctionPath) {
      ...
      renderFunctionPath.traverse({
        ...
        VariableDeclarator(path) {
          ...
          const declarationPath = path.findParent((parentPath) =>
          ...
          for (const property of fetchExpression.callExpression.arguments[0]
            .properties) {
            if (property.type === "ObjectProperty")
              fetcherProperties.push(property);
          }
          ...
          if (path.node.id.type === "ObjectPattern") {
            for (const property of path.node.id.properties) {
              ...
              if (property.value.type === "Identifier")
                ...
                extraRenderProperties.push({
                  key: property.key.name,
                  local: property.value.left.name,
                });
            }
          ...
          if (path.node.id.type === "Identifier") {
            const objectProperties =
              fetchExpression.callExpression.arguments[0].properties
                .filter(
            ...
            declarationPath.replaceWith(
              types.variableDeclaration("const", [
                types.variableDeclarator(
                  ...
                  fetchExpression.wrapExpression(
      ...
      for (const { key, local } of extraRenderProperties)
        addObjectPatternProperty(renderParam, key, local);

      for (const name of [...importedServices].sort((a, b) =>
        a.localeCompare(b),
      ))
      ...
      if (routeCallPath) {
        ...
        const maybeSetupArg =
          ...
          (args[1].type === "ArrowFunctionExpression" ||
            ...
            ? args[1]
            : null;
        const maybeOptionsArg =
          args.length >= 3 && args[1].type === "ObjectExpression"
            ? args[1]
            : null;
        ...
        if (setupNeeded || maybeOptionsArg || maybeSetupArg) {
          ...
          if (maybeSetupArg) {
            const existingReturnedObject =
              getReturnedObjectExpression(maybeSetupArg);
          ...
          if (maybeOptionsArg)
            setupProperties.push(
              ...prefixOptionsObject(maybeOptionsArg).properties,
            );
          ...
          if (maybeSetupArg) {
            ...
            if (existingSetupParam) {
              for (const property of existingSetupParam.properties) {
                ...
                const propertyKey = getObjectPropertyKeyName(property);
                if (propertyKey && existingSetupDataKeys.has(propertyKey))
                  continue;
          ...
          for (const property of fetcherProperties)
            pushUniqueObjectProperty(setupProperties, property);

          const setupParam = types.objectPattern(setupParamProperties);
          for (const name of [
            ...new Set([...importedServices, ...setupContextNames]),
  ...
  console.log(
    `[refactor-client-pages] ${repoRoot}: changed ${changedFiles} files`,
  );
}
