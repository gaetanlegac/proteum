...
const lowerFirst = (value: string) =>
  value.length ? value[0].toLowerCase() + value.substring(1) : value;
...
const findNearestServiceRoot = (
  serviceRoots: TServiceRoot[],
  filepath: string,
) => {
  ...
  return serviceRoots
    .filter((serviceRoot) =>
      normalizedFilepath.startsWith(serviceRoot.dir.replace(/\\/g, "/") + "/"),
    )
    .sort((a, b) => b.dir.length - a.dir.length)[0];
};

const getRelativeServiceSegments = (
  serviceRootDir: string,
  filepath: string,
) => {
  const relativePath = path
    .relative(serviceRootDir, filepath)
    .replace(/\\/g, "/");
...
const findServiceAliases = (repoRoot: string) => {
  ...
  const configFiles = findFiles(
    path.join(repoRoot, "server", "config"),
  ...
  return serviceJsonFiles
    .map<TServiceRoot | null>((serviceJsonFile) => {
      ...
      return {
        alias,
        id: metas.id,
        dir: path.dirname(serviceJsonFile),
      };
    })
...
const getRouteDecoratorMeta = (decorator: types.Decorator) => {
  ...
  if (types.isStringLiteral(firstArg)) {
    ...
    if (secondArg && types.isExpression(secondArg))
      schemaExpression = secondArg;
...
const ensureNamedExport = (
  ...
  if (
    ...
    declarationPath.replaceWith(
      types.exportNamedDeclaration(declarationPath.node, []),
    );
    return;
  }
...
const buildMemberExpression = (...segments: string[]) => {
  ...
  for (const segment of segments.slice(1))
    expression = types.memberExpression(expression, types.identifier(segment));

  return expression;
};

const replaceReferencedIdentifier = (
  identifierPath: NodePath<types.Identifier>,
  binding: TImportBinding,
) => {
...
const migrateServiceFile = (
  ...
  if (
    !/@Route\(|from ['"]@app['"]|from ['"]@models['"]|from ['"]@request['"]/.test(
      code,
    )
  )
    return false;
  ...
  traverse(ast, {
    ...
    ExportDefaultDeclaration(path) {
      ...
      if (declaration.type === "ClassDeclaration" && declaration.id?.name)
        serviceClassName = declaration.id.name;
    },

    ClassMethod(path) {
      if (path.node.key.type !== "Identifier" || !path.node.decorators?.length)
        return;
      ...
      for (const decorator of path.node.decorators) {
        ...
        const schemaSource = schemaExpression
          ? generate(schemaExpression).code
          : undefined;
  ...
  traverse(ast, {
    Program(programPath) {
      for (const schemaExport of schemaExports)
        ensureNamedExport(programPath, schemaExport);
    },
  });
  ...
  if (!serviceRoot)
    throw new Error(`Unable to find the parent service root for ${filepath}`);
  ...
  const relativeServiceImportPath =
    "./" + path.basename(filepath, path.extname(filepath));
  const schemaImports = [
    ...new Set(
      routeMethods.flatMap((routeMethod) => routeMethod.schemaImports),
    ),
  ];
  const controllerClassName = getControllerClassName(
    serviceClassName,
    filepath,
  );
  const needsSchemaHelperImport = routeMethods.some((routeMethod) =>
    routeMethod.schemaSource?.includes("schema."),
  );

  const controllerMethods = routeMethods
    .map(
      (routeMethod) =>
        `    ${routeMethod.methodName}: ${JSON.stringify(routeMethod.routePath)},`,
    )
    .join("\n");
...
if (!repoRoots.length)
  throw new Error(
    "Usage: ts-node scripts/refactor-server-controllers.ts <repo-root> [repo-root...]",
  );
...
for (const repoRoot of repoRoots) {
  ...
  const serviceFiles = findFiles(
    path.join(repoRoot, "server", "services"),
    (filepath) =>
      /\.(tsx?|jsx?)$/.test(filepath) && !filepath.endsWith(".controller.ts"),
  );

  let migratedFiles = 0;

  for (const serviceFile of serviceFiles) {
    if (migrateServiceFile(repoRoot, serviceFile, serviceRoots))
      migratedFiles++;
  }

  console.log(
    `[refactor-server-controllers] ${repoRoot}: migrated ${migratedFiles} service files`,
  );
}
