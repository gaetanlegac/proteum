...
export type TStat<TDonnees extends TObjDonneesStats> = {
  date: string;
} & TDonnees;
export type TTimeStat<TDonnees extends TObjDonneesStats> = {
  time: number;
} & TDonnees;
...
export const buildStats = <TDonnees extends TObjDonneesStats>(
  ...
  if (!Array.isArray(data)) {
    ...
    throw new Error(
      `Stats data must be an array (received ${typeof data}). See console for full provided data.`,
    );
  }
  ...
  if (data.length > 0) {
    ...
    for (const { date, ...stats } of data) {
      if (date === undefined)
        throw new Error(
          `La date est absente des données statistiques. Est-elle bien spécifiée dans le SELECT ?`,
        );
    ...
    for (let timeA = start; timeA <= end; timeA += intervalTime) {
      ...
      if (groups[timeA] !== undefined)
        for (const nom in groups[timeA]) {
          // numeric value only
          if (typeof groups[timeA][nom] !== "number") continue;
