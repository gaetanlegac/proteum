...
- When a Proteum MCP client is available, call MCP `projects_list`, select the stable `projectId` for this app, and pass that `projectId` to every app-bound MCP tool such as `runtime_status`, `orient`, `instructions_resolve`, `diagnose`, `trace_latest`, `perf_top`, `perf_request`, and `logs_tail`. Use compact CLI commands when you need a reproducible shell command, validation step, or CI-like output.
