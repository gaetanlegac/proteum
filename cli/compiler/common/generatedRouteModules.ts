...
type TGeneratedClientRouteModuleOptions = {
  chunkId: string;
  filepath: string;
};
...
const resolveRouteImport = (
  ...
  const candidates = [
    ...
    ...routeModuleExtensions.map((extension) =>
      path.join(absoluteBasePath, `index${extension}`),
    ),
  ];

  return candidates.find((candidate) =>
    routeSourceFilepaths.has(normalizeFilepath(candidate)),
  );
};

const addImportedService = (
  ...
  if (
    importedServices.some(
      (importedService) =>
        importedService.importedName === importedName &&
        importedService.localName === localName,
    )
  ) {
    return;
  }
...
const collectImportedServices = (
  ...
  for (const statement of sourceFile.statements) {
    ...
    const isClientRouterImport =
      side === "client" && clientRouterImportSources.has(source);
...
const collectRouteDefinitions = (
  ...
  const importedServiceNames = new Set(
    importedServices.map((importedService) => importedService.localName),
  );
...
const normalizeRelativeImportPath = (value: string) =>
  value.startsWith(".") ? value : `./${value}`;

const rebaseRelativeModuleSpecifiers = (
  code: string,
  outputFilepath: string,
  sourceFilepath: string,
) => {
  ...
  return replacements
    ...
    .reduce(
      (currentCode, replacement) =>
        currentCode.slice(0, replacement.start) +
        replacement.value +
        currentCode.slice(replacement.end),
      code,
    );
};
...
const buildClientRegisterArgs = (
  ...
  if (routeArgs.length === 2) {
    ...
    return [
      injectedOptions,
      getNodeText(sourceFile, routeArgs[0]),
      getNodeText(sourceFile, routeArgs[1]),
    ];
  }
...
const buildRegisterStatements = (
  ...
  if (side === "client") {
    ...
    return [
      `return ${definition.serviceLocalName}.${definition.methodName}(${finalArgs.join(", ")});`,
    ];
  }
...
export const getGeneratedRouteModuleFilepath = (
...
export const writeGeneratedRouteModule = ({
  ...
  const registerStatements = buildRegisterStatements(
    sourceFile,
    side,
    definitions,
    clientRoute,
