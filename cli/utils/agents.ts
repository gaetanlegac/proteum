...
function renderEmbeddedProjectInstructions({ coreRoot }: TProjectInstructionArgs) {
    ...
    const lines = [
        ...
        '',
        'Detailed Proteum contracts are intentionally split into the files listed in the routing table below. They are not deleted; load only the file that matches the current task, or use MCP `instructions_resolve` to get the routed set.',
        '',
        '1. Start ambiguous, generated, connected, route, controller, file, or error work with `npx proteum orient <query>`.',
        '2. When an MCP client is available, prefer MCP `runtime_status`, `orient`, `instructions_resolve`, `diagnose`, `trace_latest`, `perf_top`, `perf_request`, and `logs_tail` for repeated reads.',
        '3. Read only the files returned in `mustRead` or MCP `instructions_resolve`, plus the conditional docs that match the current task.',
        '4. Use `npx proteum runtime status` before starting a dev server, so an existing tracked session can be reused.',
        '5. During `npx proteum dev`, use the read-only runtime MCP endpoint at `/__proteum/mcp` for repeated agent reads instead of spawning equivalent CLI diagnostics.',
        '6. Use `npx proteum diagnose <target>` for request-time issues before raw trace, perf, browser, or broad source search.',
        '7. Use `--full`, `--manifest`, `--events`, or MCP `detail: "full"` only when compact output says the omitted detail is needed.',
        '',
