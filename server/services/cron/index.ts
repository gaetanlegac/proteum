...
export default class CronManager extends Service<
  Config,
  Hooks,
  Application,
  Application
> {
  ...
  public get(name?: string): CronTask | typeof CronManager.taches {
    ...
    if (cron === undefined)
      throw new Error(
        `L'instance de la tâche cron ${name} n'a pas été trouvée`,
      );
    return cron;
  }
}
