...
export default class CronTask {
  ...
  public schedule(next: TFrequence) {
    ...
    if (typeof next === "string") {
      ...
      this.manager.config.debug &&
        console.info(
          `[cron][${this.nom}] Planifié pour ${this.nextInvocation.toISOString()} via cron ${next}`,
        );
      ...
      this.manager.config.debug &&
        console.info(
          `[cron][${this.nom}] Planifié pour ${this.nextInvocation.toISOString()} via date`,
        );
    }
  }
  ...
  public run(now: boolean = false) {
    ...
    const runnable =
      ...
      this.nextInvocation.valueOf() <= maintenant.valueOf();
