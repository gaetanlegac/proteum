...
export type TCompleteEmail = With<
  THtmlEmail,
  {
    to: TPerson[];
    from: TPerson;
...
- FONCTIONS
----------------------------------*/
export default abstract class Email<TConfig extends Config> extends Service<
  TConfig,
  Hooks,
  Application,
  Application
> {
  ...
  public async send(...args: TEmailSendArgs): Promise<void> {
    ...
    const emailsToSend: TCompleteEmail[] = emails.map((email) => {
      const from: TPerson =
        email.from === undefined ? this.config.default.from : email.from;
    ...
    console.info(
      LogPrefix,
      `Sending ${emailsToSend.length} emails:`,
      emailsToSend[0].subject,
    );

    // Pas d'envoi d'email quand local
    if (
      this.app.env.profile !== "prod" &&
      this.config.simulateWhenLocal === true
    ) {
