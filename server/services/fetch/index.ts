...
> {
  ...
  public async request(
    ...
    if (this.config.router === undefined)
      throw new Error(
        `Please bind the Router service to the Fetch service in order to contact APIs.`,
      );
  ...
  public async image(
    ...
    if (this.disk === undefined)
      throw new Error(
        `Please provide a Disks service in order to download files.`,
      );
    ...
    try {
      ...
      console.error(
        LogPrefix,
        `Error while fetching image at ${imageFileUrl}:`,
        error,
      );
      return null;
    }
    ...
    if (imageMod) {
      ...
      imageBuffer = await processing
        ...
        .catch((e) => {
          console.error(
            LogPrefix,
            `Error while processing image at ${imageFileUrl}:`,
            e,
          );
          return null;
        });
