...
> extends Service<TConfig, Hooks, Application> {
  ...
  public constructor(parent: AnyService, config: TConfig, app: TApplication) {
  ...
  protected async ready() {
    ...
    this.ws = new WebSocketServer({ server: this.router.http.http }).on(
      "connection",
  ...
  public closeAll() {
    ...
    for (const path in this.scopes) this.scopes[path].close();
  }
}
