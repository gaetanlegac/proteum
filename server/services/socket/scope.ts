...
export default class SocketScope<TUser extends {}> {
  ...
  public newClient(socket: WebSocket, req: IncomingMessage) {
    context.run({ channelType: "socket", channelId: this.path }, async () => {
      ...
      if (!username) {
        console.log(
          `Rejecting connection on ${this.path} for client ${socket.ip} (${socket.id})}: Not authenticated`,
        );
      ...
      if (
        this.connectEvent &&
        (await this.connectEvent(socket, req)) === false
      ) {
        return;
      }
      ...
      console.log(
        `Client ${socket.username} (${socket.ip}) connected on ${this.path}. Connections number for this user: ` +
      ...
      socket.on("close", () => {
        if (this.users[username].length > 1)
          this.users[username] = this.users[username].filter(
            (s) => s.id !== socket.id,
          );
        else this.users[username] = [];

        console.log(
          `Client ${socket.ip} (${socket.id}) disconnected from ${this.path}. Connections number for this user: ` +
            this.users[username].length,
        );
  ...
  public isConnected(username: string) {
    return (
      this.users[username] !== undefined && this.users[username].length !== 0
    );
  }

  public send(usernames: string | string[], command: string, data: any = true) {
    if (usernames === "*") usernames = Object.keys(this.users);
    else if (!Array.isArray(usernames)) usernames = [usernames];
    ...
    for (const username of usernames) {
      ...
      console.log(
        "Sent event " + command + " to " + username + " on " + this.path,
      );
    }
  }
