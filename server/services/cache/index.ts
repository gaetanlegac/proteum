...
type TCacheGetOnlyArgs = [cle: string, avecDetails: true];
...
export default class Cache extends Service<Config, Hooks, Application> {
  ...
  public constructor(parent: AnyService, config: Config, app: Application) {
  ...
  private cleanMem() {
    ...
    for (const key in this.data) {
      ...
      if (entry?.expiration && entry.expiration < now) {
        this.config.debug &&
          console.log(LogPrefix, `Delete expired data: ${key}`);
        this.del(key);
      }
    }
    ...
    for (const entryKey in this.data) {
      ...
      this.config.debug &&
        console.log(
          LogPrefix,
          `Flush ${entry.changes} changes for ${entryKey}`,
        );
  ...
  public async get<
    TValeur extends TPrimitiveValue,
    TArgs extends TCacheGetOnlyArgs | TCacheGetOrUpdateArgs<TValeur> =
      | TCacheGetOnlyArgs
      | TCacheGetOrUpdateArgs<TValeur>,
  >(
    ...args: TArgs
  ): Promise<
    | TValeur
    | CacheEntry<TValeur>
    | (TArgs extends TCacheGetOnlyArgs ? undefined : TValeur)
  > {
    let cle: string;
    ...
    if (expiration === undefined) expiration = "never";
    ...
    if (entry !== undefined) {
      this.config.debug &&
        console.log(LogPrefix, `Get "${cle}": restored via cache`);
    } else if (func !== undefined) {
      this.config.debug &&
        console.log(LogPrefix, `Get "${cle}": refresh value`);
      ...
      if (expiration !== "now") await this.set(cle, entry.value, expiration);
    } else return undefined;

    return avecDetails ? entry : (entry.value as TValeur);
  }
  ...
  private delayToTimestamp(delay: TExpirationDelay): number | undefined {
    if (delay === "now") {
      ...
      // Lifetime in seconds
    } else if (typeof delay === "number") return Date.now() + delay;
    // Date limit
    else return delay.getTime();
  }
}
