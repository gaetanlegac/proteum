...
export default class {
  ...
  public async FromGoogle(request: ServerRequest): Promise<string> {
    ...
    if (app.env.name === "local") {
      const { redirect } = await this.Auth(
        "tbsoftwares23@gmail.com",
        request,
        true,
      );
      return redirect;
  ...
  public async GoogleResponse(
    ...
    if (codeOrToken === undefined) throw new Forbidden("Bad code / token");
    ...
    this.config.debug &&
      console.log(LogPrefix, "Auth via google", googleConfig);

    let ticket: LoginTicket;
    try {
      ticket = await this.googleClient.verifyIdToken({
        idToken: codeOrToken,
        audience: [googleConfig.web.clientId, googleConfig.android.clientId],
      });
    } catch (error) {
      throw new Forbidden(
        `Google denied your login attempt: ` +
          error.message +
          `. If you don't think it's normal, please contact us.`,
      );
    }

    const payload = ticket.getPayload();
    if (payload === undefined) throw new Forbidden("Invalid payload");
    const { email, sub: google_id } = payload;

    if (email === undefined)
      throw new Forbidden(
        "Unable to get your email address from the Google sign-in.",
      );
  ...
  public async Signup(
    ...
    if (duplicates !== 0) username += duplicates;
    ...
    // Hook
    if (this.beforeSignup !== undefined) await this.beforeSignup(user);

    // Referrer
    if (user.referrer !== undefined) {
      const refExists = await this
        .sql`FROM User WHERE name = ${user.referrer}`.exists();
      if (!refExists) user.referrer = undefined;
      else {
        await this.sql.upsert(
          "UserStats",
          {
            user: user.referrer,
            date: new Date(),
  ...
  public async setReferrer(
    referrer: unknown,
    { user, response, cookies, req, detect, request }: ServerRequest,
  ) {
    ...
    if (!cookies.utm) {
      ...
      const utm = [data.utm_medium, data.utm_source, data.utm_campaign].filter(
        (a) => a !== undefined,
      );
    ...
    if (cookies.r === undefined && typeof referrer === "string") {
      ...
      const referrerExists = await this
        .sql`FROM User WHERE name = ${referrer}`.count();
      if (referrerExists === 0) return;
