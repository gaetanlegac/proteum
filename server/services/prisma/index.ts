...
export default class ModelsManager extends Service<
  Config,
  Hooks,
  Application,
  Application
> {
  ...
  public Facet<D extends TDelegate<R>, S extends TSubset, R, RT>(
  ...
  public string = (strings: TemplateStringsArray, ...data: any[]) => {
    ...
    if (typeof data === "function")
      throw new Error(
        `A function has been passed into the sql string template: ` + data,
      );
      ...
      .map((stringBefore, i) => {
        if (i <= iMax) {
          ...
          if (value === undefined || value === null) {
            ...
            if (prefix === "=")
              stringBefore =
                stringBefore.substring(0, stringBefore.length - 1) + "IS ";
            ...
            if (typeof value === "object") {
              ...
              value =
                Object.keys(value).length === 0
                  ? "1"
                  : this.equalities(value).join(keyword);
  ...
  public equalities = (data: TObjetDonnees, forStorage: boolean = false) => {
    return Object.keys(data).map(
      (k) => "" + k + " = " + this.esc(data[k], forStorage),
    );
  };
}
