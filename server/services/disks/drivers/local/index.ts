...
> extends DiskDriver<TConfig> {
  ...
  public async readDir(bucketName: TBucketName, dirname?: string) {
    ...
    for (const source of sources) {
      ...
      const csvFiles = fs.readdirSync(path.join(fullPath, parentFolder), {
        withFileTypes: true,
      });
    ...
    this.config.debug &&
      console.log(`readDir ${fullPath}: ${files.length} objects`);
    return files;
  }
  ...
  public async move(
    ...
    this.config.debug &&
      console.log(`move ${fullPathSource} to ${fullPathDestination}`);
