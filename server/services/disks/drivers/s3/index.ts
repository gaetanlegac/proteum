...
> extends DiskDriver<TConfig> {
  ...
  public readDir(bucketName: TBucketName, dirname?: string) {
    ...
    return new Promise<SourceFile[]>((resolve, reject) => {
      debug &&
        console.log(
          `readDir ` +
            (dirname === undefined ? bucket : path.join(bucket, dirname)),
        );
      this.s3.listObjectsV2({ Bucket: bucket }, async (err, data) => {
        ...
        for (const file of data.Contents) {
          ...
          const rowsCount =
            (fileContent as unknown as string).split("\n").length - 1;
        ...
        debug &&
          console.log(
            `readDir ${bucket}/${dirname || ""}: ${files.length} objects`,
          );
        resolve(files);
      });
    });
  ...
  public async move(
    ...
    const bucket = this.config.buckets[bucketName];
    debug &&
      console.log(`move ${bucket}/${source} to ${bucket}/${destination}`);
    ...
    debug &&
      console.log(`Move ${bucket}/${source} to ${bucket}/${destination}: OK`);
  }
    ...
    return new Promise((resolve, reject) => {
      this.s3.upload(
        ...
        (err, data) => {
          ...
          debug &&
            console.log(
              `outputFile ${bucket}/${filename}: OK (${data.Location})`,
            );
