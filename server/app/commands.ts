...
- SERVICE
----------------------------------*/
export default class CommandsManager extends Service<
  Config,
  Hooks,
  Application
> {
  ...
  public fromList(list: Command[]) {
    for (const command of list) {
      if (this.commandsIndex[command.name] !== undefined)
        throw new Error(
          `Tried to register command "${command.name}", but it already has been defined.`,
        );
  ...
  public async run(commandString: string) {
    ...
    this.config.debug &&
      console.log(LogPrefix, `Run command: ${commandString} | Parsed:`, {
        _,
        ...args,
      });
    ...
    for (const commandName of _) {
      const commandsList: CommandsList =
        command === undefined ? this.commandsIndex : command.childrens;
